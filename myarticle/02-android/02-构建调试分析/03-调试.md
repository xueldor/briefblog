...
如果你是高手，喜欢用命令行，那么没必要折腾，直接用命令行就行了。下一节简单介绍了一下怎么用命令行调试。不过我不是高手，只能大致介绍一下。
...
方案3：远程服务器安装了samba,因此可以在windows的文件夹里输入`\\192.168.20.111\share\`打开。假设源码的地址是`\\192.168.20.111\share\huanglixin`,映射网络驱动器为V盘。
...
* program 为你要调试的程序
* setupCommands里面，我删除了 solib-search-path ，因为在linux里面，一组路径之间用冒号分隔，而windows系统是用逗号分隔路径。这里我不知道是不是应该把冒号改成逗号。因为是“V:/out”这样的路径，之间也有一个冒号，我不知道怎么改，干脆删掉。

然后，跟前面一样，adb shell 里面 运行gdbserver ，PC端执行adb forward。

然后点击debug按钮，Samba映射到本地的文件访问非常缓慢，所以你可能要等一段时间。

Samba映射到本地驱动器的方式只用于debug，查找代码还是用remote ssh插件，因为Samba的方式非常卡顿。
...
AOSP的C/C++调试原理，就是在Android端运行gdbserver，然后PC端使用gdb通过tcp连接到gdbserver，所以只要知道原理，任何C/C++ IDE理论上都是可以进行调试的，eclipse可以，CLion可以，AS也可以, 无非就是怎么配置的问题。有些IDE比较智能，帮你配好了，有些则需要自己一步一步踩坑。前面介绍VS Code调试，就是因为谷歌官方支持生成VS Code的debug配置，操作比较简单，所以成了我的选择。

目前还不知道AS里面怎么配置。根据debug的原理，肯定需要把带symbol信息的so或可执行文件路径传给android studio。然后调试工具是NDK里的还是源码路径下的？

还是说需要创建一个gradle管理的安卓app项目，把framework的代码拷贝进去，编写CMakeList.txt？

不知道谷歌开发aosp的那些工程师是怎么做的。

Clion的debug的配置模板里面有GDB Remote Debug，所以配置容易一点，网上的教程都是针对Clion的，找不到AS的，甚至有些文章直言：“AOSP可以很容易的导入到Android Studio，很舒服的跳转、调试，但是仅限于Java，Native代码连跳转都不行。” 但是有了vs code，为什么要再搞一个Clion呢？

之所以希望能在AS里面配起来，是因为AS是谷歌官方提供的开发工具，平时开发APP用它调试java、C、甚至kotlin都非常方便，所以我觉得也应该能用它调试系统进程和源码，能自如地从java的断点跳到C的断点里面。

现在不知道怎么做，只好把java的调试和native的调试分开，用AS 调试 java，用VS Code 调试 C/C++。
